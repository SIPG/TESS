...
def write_logs(slave_ip,list_offline):
	...
	vm=paramiko.SSHClient()
        ...
        log, lines_list="",[]
        try:
                stdin,stdout,stderr = jhost.exec_command('scp /var/VRTSvcs/log/engine_A.log root@10.45.207.82:/root/faultedlogs\n')
...
for job in jobs_list:
        ...
        if jobName != None:
                ...
                responseJob = json.loads(responseJob_temp.read())
                ticks=responseJob["timestamp"]+responseJob["duration"]
                jobCompleteTime = datetime.datetime.fromtimestamp(ticks/1000)
                diff = abs(jobCompleteTime-current_date)
                ...
                if hours<2:
                        ...
                        print "***********************************************"
                        print "current_date: ",current_date
                        print "jobCompleteTime: ",jobCompleteTime
                        slave_ip=get_ip(failedJobLink+"/consoleFull")
                        print "slave_ip: ",slave_ip
                        get_faulted_services(slave_ip)
