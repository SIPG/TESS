...
def has_passwordless_access(user, server):
    rc = run_cmd("ssh -o ConnectTimeout=5 -o PreferredAuthentications=publickey -o BatchMode=yes {0}@{1} 'hostname' > /dev/null 2>&1".format(user, server), "Public key-based SSH connection test failed")
...
def key_copy(srvr,usrnme,pswrd):
	rc = copy_ssh_key_to_server(srvr, usrnme, pswrd)
