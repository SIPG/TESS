...
def modify(url_env,sheet,drop):
	...
	if count==0:
		...
		print "Modifying in Staging sheet",drop," for ",ISO
	...
	yellowFill = PatternFill(start_color='ffa500',end_color='ffa500',fill_type='solid')
	...
	for suite in suites_in_sheet:
                         if suite not in suites_in_allure and suite !=None:
                                ...
                                ws2[remove_cell].fill=yellowFill
        cell=str(chr((column_count)/26+64))+str(chr((column_count)%26+65))
        cell=cell.replace("@","")
        cell=cell+"1"
...
reruns=[]
if "xunit" in Staging_link:
        url_env = Staging_link[:-8]+"/data/environment.json"
elif "#" in Staging_link:
        url_env = Staging_link[:-3]+"/data/environment.json"
else:
        url_env = Staging_link+"/data/environment.json"
